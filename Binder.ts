 ...
 * 或者在onload, start上 使用getChildByName() 或者find() 查找结点, 又会显得代码冗长
...
class Binder {
    ...
    // 绑定node
    public bindNode(node: Node, component: UIBinder) {
    ...
    // 绑定子节点
    private _bindSubNode(node: Node, component: UIBinder) {
        // 检测前缀是否符合绑定规范
        let name = node.name;
        if(CocosHelper.checkBindChildren(name)) {
            if (CocosHelper.checkNodePrefix(name)) {
                ...
                if(names === null || names.length !== 2 || !SysDefine.SeparatorMap[names[0]]) {
                    console.log(names);
                    console.log(`${name} 命令不规范, 请使用_lab$xxx的格式!, 或者是在SysDefine中没有定义`);
                    return ;
                }
                // 未定义的类型
