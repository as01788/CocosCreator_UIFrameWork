...
import { _decorator, Rect, Node, Component, ScrollView, Graphics, v2, v3, UITransform, EventHandler, rect, color, CCFloat } from 'cc';
...
export class ScrollViewHelper extends Component {
    ...
    private _svCachedScrollView: ScrollView;
    private _rectCachedViewport: Rect;
    private _debugNode: Graphics;
    ...
    public setDebug(debug: Boolean) {
        if (debug && !this._debugNode) {
            this._debugNode = (new Node()).addComponent(Graphics);
            this.scrollView.content.addChild(this._debugNode.node);
            this._debugNode.node.setPosition(v3(0, 0, 0));
            ...
            if (loop) {
                for (let i = this._proxys.length - 1; i >= 0; i--) {
                    ...
                    if (!target.horizontalLoop) {
                        ...
                        copyed.region = new Rect(target.region.x, target.region.y, target.region.width, target.region.height);
                        copyed.horizontalLoop = true;
                        copyed.verticalLoop = target.verticalLoop;
                        copyed.region.x += this.scrollView.content.getComponent(UITransform).width;
                        this._proxys.push(copyed);
                    }
                }
                this.scrollView.content.getComponent(UITransform).setContentSize(this.scrollView.content.getComponent(UITransform).width * 2, this.scrollView.content.getComponent(UITransform).height);
            } else {
                for (let i = this._proxys.length - 1; i >= 0; i--) {
                    let target = this._proxys[i];
                    if (target.horizontalLoop) {
    ...
    onLoad() {
        this._bindScrollView(this.getComponent(ScrollView));
    }
    ...
    private _checkLoopHorizontal() {
        ...
        if (Math.abs(delta) > 2) {
            ...
            let originalWidth = this.scrollView.content.getComponent(UITransform).width / 2;
    ...
    private _bindScrollView(view: ScrollView) {
        this._svCachedScrollView = this.getComponent(ScrollView);
        ...
        this._svCachedScrollView.node.on(Node.EventType.SIZE_CHANGED, this._refreshViewportRect, this);
        this._svCachedScrollView.content.on(Node.EventType.ANCHOR_CHANGED, this._refreshViewportRect, this);
        this._svCachedScrollView.content.on(Node.EventType.SIZE_CHANGED, this._refreshViewportRect, this);
        this._refreshViewportRect();
    }
    private _refreshViewportRect() {
        this._rectCachedViewport = rect();
        this._rectCachedViewport.size = this._svCachedScrollView.node.getComponent(UITransform).contentSize;
        this._rectCachedViewport.origin = v2(-this._svCachedScrollView.content.getComponent(UITransform).anchorX * this._svCachedScrollView.content.getComponent(UITransform).width,
            (1 - this._svCachedScrollView.content.getComponent(UITransform).anchorY) * this._svCachedScrollView.content.getComponent(UITransform).height - this._rectCachedViewport.size.height);
        this._checkUIState();
        this._syncingUI = true;
    }
    ...
    private _syncUIState(maxAllocCount: number) {
        ...
        for (let proxy of this._proxys) {
            if (proxy.state == ScrollViewElementProxyState.NoUI && proxy.ui) {
                ...
                changeCount++;
                if (this._debugNode) {
                    console.log("ScrollViewHelper._syncUIState ----> free ui");
                }
            }
        }
        for (let proxy of this._proxys) {
            if (proxy.state == ScrollViewElementProxyState.HaveUI && !proxy.ui) {
                ...
                if (this._debugNode) {
                    console.log("ScrollViewHelper._syncUIState ----> alloc ui");
                }
            }
            if (maxAllocCount <= 0) {
                break;
    ...
    private _refreshDebug() {
        if (this._debugNode) {
            ...
            this._debugNode.fillColor = color(255, 0, 0, 255);
